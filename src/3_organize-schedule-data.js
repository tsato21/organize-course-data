...
const FACULTY_NAME_REPLACE = [
                    {preName: 'J.F', aftName: 'J FF'},
                    {preName: 'L.F', aftName: 'L FF'},
                    ];
...
function organizedClassScheduleDataType3() {
  ...
  let programDataSheetName = Browser.inputBox(`Input the name of the sheet where you pasted the class schedule data provided by the Program (e.g., 3_Schedule Data by Program).`,Browser.Buttons.OK_CANCEL);
  ...
  let displaySheetName = Browser.inputBox(`Input the name of the sheet where organized course offering data is displayed (e.g., 3_Organized Schedule Data).`,Browser.Buttons.OK_CANCEL);
  ...
  for (let i = 3; i < data.length; i++) { // Skip the first two rows
    ...
    for (let j = 1; j < row.length; j++) { // Start from the second column
      ...
      // let room = lines.length === 6 ? lines[5] : lines[4];
...
function processCourseRecord_(courseCode, cellColumn, rawValue, sectionCode, semesterCode, faculty, startTime, endTime, room, records) {
  ...
  if (courseCode == 'ADV150'){ //If the courseCode is "BRI150", set classCode and push the record individually
    classCode = courseCode + '-1' + semesterCode;
    console.log(`${courseCode} is a Advanced Learning Course`);
    pushCourseRecord_(records, cellColumn, rawValue, classCode, faculty, startTime, endTime, room);
  } else if (slashIncludeCourse) { //If the courseCode includes "/", split by "/" and push two records with some adjustment
...
function displayRecords_(records,header,displaySheet){
  ...
  displaySheet.getRange("A2:G").setBackground('#E7FCDB');
  displaySheet.getRange(1,1,1,header.length).setValues([header])
                                      .setFontWeight('bold').setBackground('#B4B4B4').setHorizontalAlignment("center");
